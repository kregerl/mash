...
Token Lexer::getTokenFromChar(const char& c) {
    ...
    switch (c) {
        case '+':return {str, TokenType::Addition};
        case '-':return {str, TokenType::Subtraction};
        case '*':return {str, TokenType::Multiplication};
        case '/':return {str, TokenType::Division};
        case '(':return {str, TokenType::LParen};
        case ')':return {str, TokenType::RParen};
        case '%':return {str, TokenType::Modulo};
        case '^':return {str, TokenType::Bitwisexor};
        case '&':return {str, TokenType::Bitwiseand};
        case '|':return {str, TokenType::Bitwiseor};
        case '!':return {str, TokenType::Factorial};
        case '=':return {str, TokenType::Equals};
        case ',':return {str, TokenType::Comma};
        case '[':return {str, TokenType::LBracket};
        case ']':return {str, TokenType::RBracket};
        case '{':return {str, TokenType::LBrace};
        case '}':return {str, TokenType::RBrace};
        case ':':return {str, TokenType::Colon};
        case '\"':return {str, TokenType::DoubleQuote};
        default:return {};
    }
}
    ...
    if (m_expression[index] == '0' &&
        ...
        *i = index - 1;
        if (next == 'x') {
            ...
            return {std::to_string(n), TokenType::Number, InternalType::Double};
        } else {
            bool valid = true;
...
Token Lexer::getTokenFromString(const std::string& string) {
    ...
    if (string == "**") {
        ...
        type = TokenType::KeywordHex;
    } else if (string == "binary") {
        type = TokenType::KeywordBinary;
    } else {
    ...
    for (int i = 0; i < m_expression.size(); i++) {
        ...
        switch (c) {
            ...
            case '>':
                ...
                break;
            case '.':tokens.emplace_back(readNumberToken(&i));
                break;
            default:
                if (std::isalpha(c)) {
